...
- docker file 
- unit tests not handling null,blank values of required fields on the models as the jackson annotations are taking care of it when request is recieved
...
MarketingAccountServiceImpl and MarketingContactServiceImpl - these perform the business  logic and persists the VendorAccount,VendorContactentities 
                                                              respectively to backing store..
...
AccountControllerTest,ContactControllerTest - tests the Spring-Boot REST controller
MarketingAccountServiceTest  - tests the MarketingAccountService.
MarketingContactServiceTest  - tests the MarketingContactService.
...
3. To run the application, run : "mvn spring-boot:run" and use POSTMAN collection provided to make GET/POST/PUT/DELETE requests by changing
                                 the requests accordingly.

 
4. To run unit tests, run: "mvn test"

The jar has already been built (target/marketing-app-0.0.1-SNAPSHOT.jar). If you want to rebuild the jar, run: "mvn clean install"

H2 DB: The H2 Database can be accessed at "http://localhost:8761/h2/" and click connect.
